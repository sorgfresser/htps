...
static PyMemberDef Params_members[] = {
        {"exploration",                              T_DOUBLE, offsetof(htps::htps_params,
                                                                        exploration),                                0, "exploration parameter"},
        {"num_expansions",                           T_ULONG,  offsetof(htps::htps_params,
                                                                        num_expansions),                             0, "max number of expansions"},
        {"succ_expansions",                          T_ULONG,  offsetof(htps::htps_params,
                                                                        succ_expansions),                            0, "expansions per batch"},
        {"early_stopping",                           T_BOOL,   offsetof(htps::htps_params,
                                                                        early_stopping),                             0, "early stopping flag"},
        {"no_critic",                                T_BOOL,   offsetof(htps::htps_params,
                                                                        no_critic),                                  0, "no critic flag"},
        {"backup_once",                              T_BOOL,   offsetof(htps::htps_params,
                                                                        backup_once),                                0, "backup once flag"},
        {"backup_one_for_solved",                    T_BOOL,   offsetof(htps::htps_params,
                                                                        backup_one_for_solved),                      0, "backup one for solved flag"},
        {"depth_penalty",                            T_DOUBLE, offsetof(htps::htps_params,
                                                                        depth_penalty),                              0, "depth penalty"},
        {"count_threshold",                          T_ULONG,  offsetof(htps::htps_params,
                                                                        count_threshold),                            0, "count threshold"},
        {"tactic_p_threshold",                       T_BOOL,   offsetof(htps::htps_params,
                                                                        tactic_p_threshold),                         0, "tactic p threshold flag"},
        {"tactic_sample_q_conditioning",             T_BOOL,   offsetof(htps::htps_params,
                                                                        tactic_sample_q_conditioning),               0, "tactic sample q conditioning flag"},
        {"only_learn_best_tactics",                  T_BOOL,   offsetof(htps::htps_params,
                                                                        only_learn_best_tactics),                    0, "only learn best tactics flag"},
        {"tactic_init_value",                        T_DOUBLE, offsetof(htps::htps_params,
                                                                        tactic_init_value),                          0, "tactic initial value"},
        {"policy_temperature",                       T_DOUBLE, offsetof(htps::htps_params,
                                                                        policy_temperature),                         0, "policy temperature"},
        {"effect_subsampling_rate",                  T_DOUBLE, offsetof(htps::htps_params,
                                                                        effect_subsampling_rate),                    0, "effect subsampling rate"},
        {"critic_subsampling_rate",                  T_DOUBLE, offsetof(htps::htps_params,
                                                                        critic_subsampling_rate),                    0, "critic subsampling rate"},
        {"early_stopping_solved_if_root_not_proven", T_BOOL,   offsetof(htps::htps_params,
                                                                        early_stopping_solved_if_root_not_proven),   0, "early stopping solved flag"},
        {"virtual_loss",                             T_ULONG,  offsetof(htps::htps_params,
                                                                        virtual_loss),                               0, "virtual loss"},
        {NULL}
};
...
static int Theorem_init(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    static const char *kwlist[] = {"conclusion", "unique_string", "hypotheses", "context", "past_tactics", "metadata",
                                   NULL};
...
static PyObject *Theorem_get_hypotheses(PyObject *self, void *closure) {
    ...
    for (size_t i = 0; i < thm->cpp_obj->hypotheses.size(); i++) {
        ...
        PyObject *args = Py_BuildValue("ss", thm->cpp_obj->hypotheses[i].identifier.c_str(),
                                       thm->cpp_obj->hypotheses[i].type.c_str());
        if (!args) {
            return NULL;
        }
...
static PyObject *Theorem_get_past_tactics(PyObject *self, void *closure) {
    ...
    for (size_t i = 0; i < thm->cpp_obj->past_tactics.size(); i++) {
        PyObject *args = Py_BuildValue("sOn", thm->cpp_obj->past_tactics[i].unique_string.c_str(),
                                       thm->cpp_obj->past_tactics[i].is_valid ? Py_True : Py_False,
                                       (Py_ssize_t) thm->cpp_obj->past_tactics[i].duration);
        if (!args) {
            return NULL;
        }
